  ...
  <div class="tool-body">
    <div class="wrap">
          ...
          <div class="lm-map" id="lmMap" aria-live="polite">
            ...
            <div class="lm-grid">
              ...
              <div><label for="lmUnits">File units</label>
                <select id="lmUnits"><option value="yards">Yards</option><option value="meters">Metres</option></select></div>
            </div>
      ...
      <section class="panel" aria-labelledby="log-h">
        ...
        <div class="so-wrap">
          ...
          <div class="so-readout" aria-live="polite" aria-atomic="true">
            ...
            <span class="unit" id="unitLabel">yards carry</span><!-- follows the Locker profile -->
          </div>
            ...
            <table class="so-tbl" id="statTbl">
              <thead>
                <tr>
                  ...
                  <th scope="col">Median <span id="thMedianU">(yd)</span></th>
                  <th scope="col">Spread</th>
                  <th scope="col">80% band</th>
                  <th scope="col">Gap <span id="thGapU">(yd)</span></th>
                </tr>
              </thead>
      ...
      <section class="panel explain">
        ...
        <p>Both numbers are yards. Log in metres and the tool holds them at 7.3 and 22.9 metres, so a
          25-yard hole in the bag stays a 25-yard hole whichever unit you read.</p>
      ...
      <section class="faq-block panel" aria-labelledby="faq-h">
        ...
        <details><summary>Can I import my launch monitor data?</summary>
          <p>Yes. Export the session as a CSV from TrackMan, Foresight, Garmin, FlightScope, Rapsodo or
            ...
            carry columns, converts metres if the file uses them, skips the summary rows vendors append,
...
SCRIPT = r'''  <script>
    ...
    var GAP_HOLE = 25;        /* yards: median-to-median gap above this = no club for it */
    var GAP_DUP = 8;          /* yards: below this = two clubs doing one job */
    var MAX_SHOTS = 50;
    var MAX_CARRY = 500;

    /* ==================== UNITS ==================== */
    /* Every shot is stored in the unit the Locker profile says (GolfrawLocker
       .setUnits converts the stored numbers when that changes), so the logger
       works in whichever unit the golfer chose and only the words change.
       The two gap thresholds are yards, like the Bag Audit's: 25 yards stays
       25 yards (22.9 metres) rather than silently becoming 25 metres. */
    var M_PER_YD = 0.9144;
    var profileUnits = 'yards';                    /* 'yards' | 'meters' */
    function metric() { return profileUnits === 'meters'; }
    function uShort() { return metric() ? 'm' : 'yd'; }
    function uWord(n) {
      if (metric()) return n === 1 ? 'metre' : 'metres';
      return n === 1 ? 'yard' : 'yards';
    }
    /* A yard threshold in the unit on screen, one decimal in metres. */
    function thr(yd) { return metric() ? Math.round(yd * M_PER_YD * 10) / 10 : yd; }
    /* ...and how to say it, keeping the yard figure visible in metric. */
    function thrWords(yd) {
      var v = thr(yd);
      return metric() ? v + ' ' + uWord(v) + ' (' + yd + ' yards)' : v + ' ' + uWord(v);
    }
    /* Adopt the profile's unit. Runs at boot and again whenever the Locker
       drawer (or another tool) changes it. Only words change here: the stored
       numbers were already converted by GolfrawLocker.setUnits. */
    function applyUnits(units) {
      profileUnits = units === 'meters' ? 'meters' : 'yards';
      var lab = $('unitLabel');
      if (lab) lab.textContent = uWord(2) + ' carry';
      var cells = ['thMedianU', 'thGapU'];
      for (var i = 0; i < cells.length; i++) {
        var c = $(cells[i]);
        if (c) c.textContent = '(' + uShort() + ')';
      }
    }
    ...
    function verdicts(rows) {
      ...
      for (var i = 0; i < rows.length - 1; i++) {
        ...
        var hole = thr(GAP_HOLE), dup = thr(GAP_DUP);
        if (gap > hole) {
          out.push({ kind: 'gap', title: 'Hole in the bag',
            text: r0(gap) + ' ' + uWord(r0(gap)) + ' between your ' + a.name + ' (' + r0(a.median) + ') and your ' +
              b.name + ' (' + r0(b.median) + '). Anything landing in between has no club for it.' });
        } else if (gap < dup) {
          out.push({ kind: 'dup', title: 'Two clubs, one job',
            text: a.name + ' and ' + b.name + ' are ' + r0(gap) + ' ' + uWord(r0(gap)) + ' apart. One of them is ' +
        ...
        if (gap >= dup && gap <= hole && a.solid && b.solid && a.p10 < b.p90) {
          out.push({ kind: 'dup', title: 'Overlapping dispersion',
            text: a.name + ' and ' + b.name + ' are ' + r0(gap) + ' ' + uWord(r0(gap)) + ' apart on the median, but ' +
      ...
      if (!out.length && rows.length > 1) {
        out.push({ kind: 'ok', title: 'Ladder looks clean',
          text: 'No gaps over ' + thrWords(GAP_HOLE) + ' and no two clubs inside ' + thrWords(GAP_DUP) +
            ' of each other. Nothing in this session needs changing.' });
      }
      return out;
    }
    ...
    function chartSvg(rows) {
      ...
      var p = [];
      var chartTitle = 'Carry distance dispersion by club, in ' + uWord(2);
      p.push('<svg class="so-chart" viewBox="0 0 ' + W + ' ' + H + '" role="img" ' +
        'aria-label="' + chartTitle + '" xmlns="http://www.w3.org/2000/svg">');
      p.push('<title>' + chartTitle + '</title>');
    ...
    /* The Locker drawer can switch yards and metres while this page is open.
       setUnits converts every stored shot, which leaves the session held here
       in the old unit: re-read it before the next tap writes yards back over
       metres. The re-read joins the write chain so it cannot overtake a save
       already in flight. */
    function syncUnits() {
      if (!L) return;
      L.getProfile().then(function (p) {
        var u = p && p.units === 'meters' ? 'meters' : 'yards';
        if (u === profileUnits) return;
        applyUnits(u);
        writeChain = writeChain.then(function () {
          return L.getOrStartSession();
        }).then(function (s) {
          session = s;
          if (!session.clubs) session.clubs = [];
          renderClubs(); renderShots(); renderReadout(); renderBar();
          if (lastRows.length) showResults(true);
        })['catch'](function () { /* the chain must survive for the next tap */ });
      })['catch'](function () { });
    }
    ...
    function renderShots() {
      ...
      for (var i = 0; i < e.shots.length; i++) {
        html.push('<span class="so-shot">' + e.shots[i] +
          ...
          ' ' + uWord(1) + ' shot">&times;</button></span>');   /* "150 yard shot", attributive */
      }
      host.innerHTML = html.join('');
    }
    ...
    /* quiet: a re-render after a unit change, not a golfer asking for results.
       It must not scroll the page under them or count a second completion. */
    function showResults(quiet) {
    ...
    function buildPrint() {
      var d = new Date();
      $('printSub').textContent = 'Median carry distances in ' + uWord(2) + ', measured ' +
        d.toLocaleDateString() + ' · ' + lastRows.length + ' clubs';
      $('printChart').innerHTML = chartSvg(lastRows);
      var rowsHtml = ['<table><thead><tr><th>Club</th><th>Shots</th><th>Median (' + uShort() + ')</th>' +
        '<th>Spread</th><th>80% band</th><th>Gap (' + uShort() + ')</th></tr></thead><tbody>'];
    ...
    /* The import target is the profile's unit (see UNITS above), so an
       imported file lands in the same unit as a tapped-in shot. */
    ...
    function lmRefresh(unitsUnknown, sourceName) {
      ...
      if (unitsUnknown) warn.push('<b>The file does not say whether it is in yards or metres.</b> It is being read as ' + ($('lmUnits').value === 'meters' ? 'metres' : 'yards') + ' — change that above if it is wrong.');
    ...
    function boot() {
      ...
      if (L) {
        L.subscribe(function (kind) {
          ...
          else if (kind === 'profile') syncUnits();
        });
      }
    }
