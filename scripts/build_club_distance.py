...
MODEL_VER = '1'   # bump when lib/distance/club-distance.js changes (immutable .js cache)
    ...
    <div class="wrap">
      ...
      <section class="panel cd-panel" aria-labelledby="cd-in-h" data-gr-inputs>
        ...
        <div class="panel-in">
          ...
          <fieldset class="cd-seg cd-units">
            <legend class="cd-legend">Units</legend>
      ...
      <section class="panel explain" aria-labelledby="cd-carry-h">
        <div class="panel-in">
          ...
          <p>Switch units and everything converts: your input, every club, the course length and swing speed
            (km/h). One yard is 0.9144 metres, so a 150-yard carry is about 137 metres and a 6,000-yard course is
            about 5,490 metres. Many courses in Australia and Europe are measured in metres; use whichever your
            card uses.</p>

          <h3>What it returns for three golfers</h3>
          <table class="cd-examples">
            <caption class="cd-sr">Plan-on carry ranges in yards for three example golfers, from their typical 7-iron carry</caption>
            ...
            <tbody>
              <tr><td>120 yards</td><td>158&ndash;193</td><td>134&ndash;145</td><td>92&ndash;103</td></tr>
              <tr><td>145 yards</td><td>195&ndash;238</td><td>161&ndash;175</td><td>107&ndash;121</td></tr>
              <tr><td>170 yards</td><td>238&ndash;291</td><td>185&ndash;201</td><td>127&ndash;143</td></tr>
            </tbody>
          </table>
          <p class="cd-hint">Yards, carry, plan-on ranges straight from the calculator. The driver ranges are wider
...
SCRIPT = r'''  <script>
    ...
    (function () {
      ...
      var M_PER_YD = 0.9144, KMH_PER_MPH = 1.609344;
      var state = { anchor: 'iron_carry', unit: 'yd', last: null, used: null, hidden: {} };
      ...
      // Model units (yards, mph) <-> what the golfer sees.
      function toModel(v) {
        if (state.anchor === 'swing_speed') return state.unit === 'm' ? v / KMH_PER_MPH : v;
        return state.unit === 'm' ? v / M_PER_YD : v;
      }
      function fromModel(v, isSpeed) {
        if (isSpeed) return state.unit === 'm' ? v * KMH_PER_MPH : v;
        return state.unit === 'm' ? v * M_PER_YD : v;
      }
      function limits() {
        var l = D.LIMITS[state.anchor];
        var sp = state.anchor === 'swing_speed';
        return [Math.round(fromModel(l[0], sp)), Math.round(fromModel(l[1], sp))];
      }
      ...
      function course(yd) { return thousands(Math.round(fromModel(yd, false) / 50) * 50); }
      ...
      function read() {
        ...
        var raw = $('cdValue').value.trim().replace(',', '.');
        if (raw === '') return { error: 'Enter ' + COPY[state.anchor].label.toLowerCase() + ' first.' };
        var v = parseFloat(raw);
        var l = limits();
        var unit = state.anchor === 'swing_speed' ? speedUnit() : distWord();
        if (!isFinite(v)) return { error: 'That is not a number we can use.' };
        if (v < l[0] || v > l[1]) {
          return { error: 'This works from ' + l[0] + ' to ' + l[1] + ' ' + unit + '. ' +
            (v < l[0] && state.unit === 'yd' && state.anchor !== 'swing_speed' ? 'Did you mean metres?' :
             v > l[1] && state.unit === 'm' && state.anchor !== 'swing_speed' ? 'Did you mean yards?' : 'Check the number.') };
        }
        return { anchor: state.anchor, value: toModel(v) };
      }
      ...
      function render() {
        ...
        if (res.tee) {
          $('cdTee').hidden = false;
          $('cdTeeRange').textContent = course(res.tee.course[0]) + '–' + course(res.tee.course[1]) + ' ' + distWord() +
      ...
      function setUnit(next) {
        ...
        var v = parseFloat($('cdValue').value.replace(',', '.'));
        var wasSpeed = state.anchor === 'swing_speed';
        if (isFinite(v)) {
          // Convert what is already typed so switching units never means retyping.
          var model = wasSpeed ? (state.unit === 'm' ? v / KMH_PER_MPH : v) : (state.unit === 'm' ? v / M_PER_YD : v);
          state.unit = next;
          $('cdValue').value = Math.round(fromModel(model, wasSpeed));
        } else {
          state.unit = next;
        }
        syncInputs();
      ...
      function boot() {
        ...
        for (var u = 0; u < units.length; u++) units[u].addEventListener('change', function (e) { setUnit(e.target.value); });
...
MODEL = '  <script src="/lib/distance/club-distance.js?v=%s"></script>\n' % MODEL_VER
