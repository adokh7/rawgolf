...
(function (root, doc) {
  ...
  var INK = '#0f172a', PAPER = '#f8fafc', WHITE = '#fff';
  var FLAG = '#b45309', GREY = '#475569', LINE = '#e2e8f0', FAIRWAY = '#166534';
  ...
  function injectCss() {
    ...
    var css = [
      ...
      'border:1px solid #cbd5e1;border-radius:999px;backdrop-filter:blur(16px) saturate(140%);',
      '-webkit-backdrop-filter:blur(16px) saturate(140%);font:800 12.5px/1 Inter,system-ui,sans-serif;text-transform:uppercase;',
      'letter-spacing:.055em;cursor:pointer;box-shadow:0 10px 30px rgba(15,23,42,.16);',
      'transition:transform .16s ease,background .18s ease,color .18s ease,box-shadow .18s ease}',
      '.gr-lk-launch:hover{background:' + FAIRWAY + ';color:' + WHITE + ';box-shadow:0 14px 34px rgba(15,23,42,.22)}',
      '.gr-lk-launch:active{transform:translateY(1px) scale(.99)}',
      '.gr-lk-launch:focus-visible{outline:3px solid #147a3b;outline-offset:3px}',
      '.gr-lk-launch .gr-lk-count{display:inline-flex;align-items:center;justify-content:center;background:#f0fdf4;color:' + FAIRWAY + ';',
      ...
      'color:' + INK + ';border:1px solid #cbd5e1;border-radius:20px;box-shadow:0 24px 70px rgba(9,24,18,.24);display:flex;flex-direction:column;overflow:hidden;',
      ...
      '.gr-lk-x:hover{background:#f0fdf4;color:' + FAIRWAY + '}',
      '.gr-lk-x:focus-visible{outline:3px solid #147a3b;outline-offset:2px}',

      '.gr-lk-body{flex:1;overflow-y:auto;-webkit-overflow-scrolling:touch;padding:18px;',
      'padding-bottom:calc(28px + env(safe-area-inset-bottom,0px));scrollbar-color:#cbd5e1 transparent;scrollbar-width:thin}',
      ...
      '.gr-lk-card{background:' + WHITE + ';border:1px solid ' + LINE + ';border-radius:14px;padding:12px 14px;box-shadow:0 1px 2px rgba(15,23,42,.04)}',
      ...
      '.gr-lk-num{font-family:"Plus Jakarta Sans",Inter,system-ui,sans-serif;font-weight:800;font-variant-numeric:tabular-nums;letter-spacing:-.02em}',
      ...
      'color:' + INK + ';border:1px solid #cbd5e1;border-radius:10px;font:600 15px/1.2 Inter,system-ui,sans-serif}',
      '.gr-lk-f input:focus,.gr-lk-f select:focus{outline:3px solid #147a3b;outline-offset:2px;border-color:#147a3b}',
      ...
      'font:800 12px/1 Inter,system-ui,sans-serif;text-transform:uppercase;letter-spacing:.06em;',
      'box-shadow:0 1px 2px rgba(15,23,42,.12);transition:transform .16s ease,background .18s ease,color .18s ease,box-shadow .18s ease}',
      '.gr-lk-btn:hover{background:#0f172a;border-color:#0f172a;box-shadow:0 6px 18px rgba(15,23,42,.16)}',
      '.gr-lk-btn:active{transform:translateY(1px) scale(.99)}',
      '.gr-lk-btn:focus-visible{outline:3px solid #147a3b;outline-offset:2px}',
      '.gr-lk-btn.ghost{background:' + WHITE + ';color:' + FAIRWAY + ';border-color:#cbd5e1;box-shadow:none}',
      '.gr-lk-btn.ghost:hover{background:#f0fdf4;color:' + FAIRWAY + ';border-color:#94a3b8}',
      '.gr-lk-btn.danger{background:' + WHITE + ';color:' + FLAG + ';border-color:#fde68a;box-shadow:none}',
      '.gr-lk-btn.danger:hover{background:' + FLAG + ';color:' + WHITE + '}',
      '.gr-lk-btn[disabled]{opacity:.45;cursor:not-allowed}',
      '.gr-lk-btn[disabled]:hover{background:#e2e8f0;border-color:' + LINE + ';color:#475569;box-shadow:none}',
      ...
      '.gr-lk-msg{margin-top:10px;padding:10px 12px;border:1px solid #cbd5e1;border-radius:10px;background:' + WHITE + ';',
      'font-size:13px;font-weight:600;line-height:1.45;display:none}',
      '.gr-lk-msg.on{display:block}',
      '.gr-lk-msg.bad{border-color:#fde68a;color:#92400e;background:#fffbeb}',
      '.gr-lk-msg.good{border-color:#86efac;color:#166534;background:#f0fdf4}',
  ...
  function render(profile, bag, rounds, bags) {
    ...
    if (!n) {
      ...
      /* Carries are stored in the profile's own unit (setUnits converts them),
         so the label follows the profile: 137.2 stored under metres is
         "137.2 m", never "137.2 yd". */
      var unit = profile.units === 'meters' ? 'm' : 'yd';
      var rows = [];
      for (var j = 0; j < bag.clubs.length; j++) {
        var c = bag.clubs[j];
        if (!c.name || !c.carry) continue;
        rows.push('<div class="gr-lk-row"><b>' + esc(c.name) + '</b>' +
          '<span class="gr-lk-num">' + esc(c.carry) + ' ' + unit + '</span></div>');
      }
      ...
      '<div class="gr-lk-sec">' +
        ...
        '<div class="gr-lk-f"><label for="gr-lk-units">Distance units</label>' +
          '<select id="gr-lk-units">' +
            ...
            '<option value="meters"' + (profile.units === 'meters' ? ' selected' : '') + '>Metres</option>' +
          '</select></div>' +
  ...
  function wire() {
    $('gr-lk-save').addEventListener('click', function () {
      ...
      /* Every field is read now, before anything async: setUnits below emits a
         store event, that refresh re-renders this panel, and the inputs the
         golfer typed into are gone by the time the save runs. */
      var units = $('gr-lk-units').value;
      var profile = {
      ...
      /* A units change converts the stored distances (setUnits), then the
         rest of the profile is saved as before. */
      var switchUnits = L.setUnits ? L.setUnits(units) : Promise.resolve();
      switchUnits.then(function () {
        return L.saveProfile(profile);
      }).then(function () {
        /* Saving emits a store event, and the refresh it triggers replaces
           this panel's HTML. Write the confirmation after that re-render, or
           it lands in a message box that is about to be thrown away: the live
           region announced it, but nobody watching the screen saw a thing. */
        return refresh();
      }).then(function () {
