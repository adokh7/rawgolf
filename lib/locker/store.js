...
(function (root) {
  ...
  /* v2 added the `sessions` store for the range logger. onupgradeneeded creates
     any store that is missing, so an existing v1 database upgrades in place
     without touching the records already in it. */
  var DB_VERSION = 2;
  var STORES = ['profile', 'bags', 'rounds', 'toolState', 'sessions', 'meta'];
  var MIRROR_KEY = 'golfraw_locker_mirror';
  var EXPORT_FORMAT = 'golfraw.locker';
  /* v2 exports carry `sessions`. A v1 file still imports: the field simply
     defaults to empty, which is why the version check is `>` and not `!==`. */
  ...
  function exportLocker() {
    return ready().then(function () {
      return Promise.all([getProfile(), listBags(), listRounds(), adapter.getAll('toolState'), listSessions()]);
    }).then(function (res) {
      ...
      if (mode === 'replace') {
        chain = chain.then(function () {
          return Promise.all([a.clear('bags'), a.clear('rounds'), a.clear('toolState'),
            a.clear('sessions'), a.clear('profile')]);
        });
      }
  ...
  function clearAll() {
    return ready().then(function (a) {
      return Promise.all([a.clear('profile'), a.clear('bags'), a.clear('rounds'),
        a.clear('toolState'), a.clear('sessions')]);
  ...
  root.GolfrawLocker = {
    ...
    limits: { MAX_CLUBS: MAX_CLUBS, MAX_ROUNDS: MAX_ROUNDS, MAX_SHOTS: MAX_SHOTS }
  };
})(window);
