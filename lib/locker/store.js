...
(function (root) {
  ...
  /* v2 exports carry `sessions`, v3 adds `scorecards`, v4 adds the Round
     Card's fields on them (penalties, 9-hole cards, summary). Older files
     still import: missing fields default to empty, which is why the version
     check is `>` and not `!==`. */
  var EXPORT_VERSION = 4;
  ...
  /* One hole as tapped in after the round. Every field is nullable because a
     ...
     `pen` (penalty strokes) and app 'miss' (green missed, side not recorded)
     arrived with the Round Card, card schema v2. null always means "not
     recorded", never zero. */
  var HoleSchema = S.object({
    ...
    app: S.oneOf(['hit', 'miss', 'short', 'long', 'left', 'right']).nullable().def(null),
    putts: S.integer({ min: 0, max: 6 }).nullable().def(null),
    pen: S.integer({ min: 0, max: 6 }).nullable().def(null)
  });

  /* Derived from the holes by the Round Card on every save, so a list of
     rounds can be drawn without re-reading every hole. Never trusted over the
     holes themselves. */
  var RoundSummarySchema = S.object({
    holes: S.integer({ min: 9, max: 18 }),
    gross: S.integer({ min: 9, max: 270 }),
    par: S.integer({ min: 27, max: 108 }),
    toPar: S.integer({ min: -100, max: 250 }),
    blowups: S.integer({ min: 0, max: 18 }),
    blowupToPar: S.integer({ min: 0, max: 250 }),
    putts: S.integer({ min: 0, max: 108 }).nullable().def(null),
    pen: S.integer({ min: 0, max: 108 }).nullable().def(null),
    gir: S.integer({ min: 0, max: 18 }).nullable().def(null),
    detail: S.oneOf(['quick', 'detailed'])
  });

  /* v1 is the Tendency Engine's card (18 holes). v2 is the Round Card's: 9 or
     18 holes, penalties, createdAt and the summary above. Both live in the
     same store so every round is one history. */
  var ScorecardSchema = S.object({
    id: S.string({ min: 1, max: 64 }),
    v: S.integer({ min: 1, max: 2 }).def(1),
    course: S.string({ max: 80 }).def(''),
    playedAt: S.integer({ min: 0 }).def(0),
    createdAt: S.integer({ min: 0 }).def(0),
    holes: S.arrayOf(HoleSchema, { max: HOLES }).def([]),
    summary: RoundSummarySchema.nullable().def(null),
    updatedAt: S.integer({ min: 0 }).def(0)
  });
  ...
  /* The profile's `units` says what unit every stored distance is recorded in
     (bag carries, range-session shots). Switching it therefore converts those
     numbers instead of relabelling them: 150 yards must not become 150 metres.
     Every record is converted and validated before the first write, and the
     profile is saved last. One decimal is kept: switching back and forth can
     move a value by a tenth at most, and it never accumulates (tested over 200
     round trips). The schema's 1-500 range is enforced at the edges. */
  var M_PER_YD = 0.9144;
  function setUnits(units) {
    if (units !== 'yards' && units !== 'meters') return Promise.reject(new Error('Units must be yards or meters.'));
    return getProfile().then(function (cur) {
      if (cur.units === units) return cur;
      var f = units === 'meters' ? M_PER_YD : 1 / M_PER_YD;
      var conv = function (v) {
        if (v === null || v === undefined) return v;
        return Math.min(500, Math.max(1, Math.round(v * f * 10) / 10));
      };
      return Promise.all([listBags(), listSessions()]).then(function (res) {
        var bags = res[0].map(function (bag) {
          var next = S.clone(bag);
          for (var i = 0; i < next.clubs.length; i++) next.clubs[i].carry = conv(next.clubs[i].carry);
          return validated(BagSchema, next, 'Bag');
        });
        var sessions = res[1].map(function (ses) {
          var next = S.clone(ses);
          for (var i = 0; i < next.clubs.length; i++) next.clubs[i].shots = next.clubs[i].shots.map(conv);
          return validated(RangeSessionSchema, next, 'Session');
        });
        return ready().then(function (a) {
          var jobs = [];
          for (var i = 0; i < bags.length; i++) jobs.push(a.put('bags', bags[i]));
          for (var j = 0; j < sessions.length; j++) jobs.push(a.put('sessions', sessions[j]));
          return Promise.all(jobs);
        }).then(function () {
          if (bags.length) emit('bags');
          if (sessions.length) emit('sessions');
          return saveProfile({ units: units });
        });
      });
    });
  }
  ...
  function allScored(card) {
    if (!card || !card.holes || !card.holes.length) return false;
  ...
  /* A card counts as finished once all 18 holes have a score. The Tendency
     Engine and the Coach Report average per 18-hole round, so a 9-hole Round
     Card round is left out of listCompletedScorecards() on purpose. */
  function scorecardComplete(card) {
    return !!card && !!card.holes && card.holes.length >= HOLES && allScored(card);
  }
  ...
  /* Resume the newest unfinished card, otherwise report none. Deliberately not
     ...
     page would pollute the trend history. A fully scored 9-hole round is
     finished, not a draft: without this the Tendency Engine would open it and
     pad it out to 18 holes. */
  function getDraftScorecard() {
    return listScorecards().then(function (all) {
      for (var i = 0; i < all.length; i++) {
        if (!allScored(all[i])) return all[i];
      }
      return null;
    });
  }
  ...
  /* ---- meta: small keyed values that are not part of the reader's golf data.
     The Pro pass lives here. Deliberately excluded from export/import: a pass
     is a credential for this person's browsers, not part of a bag or a
     round, and must not ride along in a JSON file that gets shared. */
  function getMeta(key) {
    return ready().then(function () { return adapter.get('meta', String(key)); })
      .then(function (rec) { return rec ? rec.value : null; });
  }
  function setMeta(key, value) {
    return ready().then(function () { return adapter.put('meta', { key: String(key), value: value, at: now() }); })
      .then(function () { emit('meta'); return value; });
  }
  function deleteMeta(key) {
    return ready().then(function () { return adapter.del('meta', String(key)); })
      .then(function () { emit('meta'); });
  }
