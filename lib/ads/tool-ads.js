...
(function (root) {
  ...
  /* Known: the CMP answered, or its script has loaded, failed or timed out
     without a TCF API (outside Europe the API answers gdprApplies:false). */
  function consentKnown() {
    watchTcf();
    if (tcf === 'resolved') return true;
    if (tcf === 'pending') return false;
    return consentReady;
  }
