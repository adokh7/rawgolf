...
(function (root) {
  ...
  var UNITS = { after_result: '8457096514', lower: '1432433875' };
  /* True only once Auto ads are verified absent on every PAGES slug. On
     22 Sep 2026 the four pilots were on the exclusions list, yet live checks
     still found Auto ads (in-page and anchor) on /tools-tee-box-check once
     adsbygoogle.js loaded, with or without ?client=. Off for readers until a
     QA browser (gr_adtest=1, below) shows none. */
  ...
  /* AdSense may hold a unit below the fold until it nears the screen, so the
     no-answer timer only starts once the slot is actually on screen. */
  var FILL_TIMEOUT = 12000;
  /* A browser that sets localStorage gr_adtest = '1' asks AdSense for test ads
     (data-adtest="on"): impressions and clicks from QA are not counted. Such a
     browser also skips the AUTO_ADS_EXCLUDED gate (never the Pro or PAGES
     gates), so the pilots can be checked live before readers get them. It
     changes nothing for anyone else. */
  var ADTEST = false;
  try { ADTEST = root.localStorage.getItem('gr_adtest') === '1'; } catch (e) { ADTEST = false; }
  ...
  function decide() {
    ...
    if (!AUTO_ADS_EXCLUDED && !ADTEST) return 'auto_ads_not_excluded';
  ...
  /* In the EEA, the UK and Switzerland Google's message may still be open
     when a result appears. AdSense waits for the choice before requesting an
     ad; the slots wait with it and take no space, so an unanswered message
     never leaves an empty box, and a unit never fills later out of sight.
     The choice itself is a click, so the slot opening then does not count as
     layout shift. */
  var tcf = 'unknown';                     // unknown | pending | resolved
  function watchTcf() {
    if (tcf !== 'unknown' || typeof root.__tcfapi !== 'function') return;
    tcf = 'pending';
    try {
      root.__tcfapi('addEventListener', 2, function (d, ok) {
        if (!ok || !d || tcf === 'resolved') return;
        if (d.gdprApplies === false || d.eventStatus === 'tcloaded' || d.eventStatus === 'useractioncomplete') {
          tcf = 'resolved';
          try { check(); } catch (e) { /* never break the page */ }
        }
      });
    } catch (e) { tcf = 'resolved'; }
  }
  /* Known: the CMP has answered. Google's loader script finishes first and
     defines __tcfapi a moment later, so after it loads the slots keep asking
     for up to 8s; only then is the CMP treated as blocked and AdSense left to
     apply its own rules. Outside Europe the API answers gdprApplies:false. */
  var tcfGiveUp = 0, tcfPoll = null;
  function consentKnown() {
    watchTcf();
    if (tcf === 'resolved') return true;
    if (tcf === 'pending' || !consentReady) return false;
    if (!tcfGiveUp) {
      tcfGiveUp = Date.now() + 8000;
      tcfPoll = setInterval(function () {
        if (tcf !== 'unknown' || Date.now() >= tcfGiveUp) { clearInterval(tcfPoll); tcfPoll = null; }
        try { check(); } catch (e) { /* never break the page */ }
      }, 250);
    }
    return Date.now() >= tcfGiveUp;
  }
  ...
  function css() {
    ...
    s.textContent =
      ...
      /* Rendered: a quiet rule, the label, and space reserved for the unit
         so the fill does not push the page around. Units are rectangles
         (300x250 or 336x280), never full-width: a responsive "auto" unit came
         back 375x812 on a phone, a whole screen of ad under a result. The top
         margin keeps a dead zone between the result's buttons and the ad. */
      '.gr-ad{margin:40px 0 28px;padding-top:14px;border-top:1px solid var(--line,#DADDD4);min-height:318px;text-align:center}' +
      '.gr-ad__label{margin:0 0 8px;text-align:left;font:600 10px/1.2 var(--gr-meta,Inter,system-ui,sans-serif);letter-spacing:.16em;' +
      'text-transform:uppercase;color:#5B665E}' +
      '.gr-ad ins.adsbygoogle{display:block;width:100%;min-height:280px}' +
  ...
  /* Calls fn once, the first time el is at least partly in the viewport. */
  function whenOnScreen(el, fn) {
    if (!root.IntersectionObserver) { fn(); return; }
  ...
  function render(slot) {
    ...
    el.innerHTML = '<p class="gr-ad__label" id="' + id + '">' + LABEL + '</p>' +
      ...
      UNITS[slot.placement] + '" data-ad-format="rectangle" data-full-width-responsive="false"' +
      (ADTEST ? ' data-adtest="on"' : '') + '></ins>';
    ...
    withAds(function (ok) {
      ...
      // AdSense requests nothing while the tab is in the background, so only
      // visible time counts: hiding the tab stops the clock, and coming back
      // restarts it.
      var timer = null;
      var arm = function () {
        clearTimeout(timer);
        if (doc.hidden || slot.state !== 'requested') return;
        timer = setTimeout(function () { if (!ins.getAttribute('data-ad-status')) collapse(slot); }, FILL_TIMEOUT);
      };
      whenOnScreen(el, function () {
        arm();
        if (doc.addEventListener) doc.addEventListener('visibilitychange', arm);
      });
    });
  }
  ...
  function check() {
    ...
    for (var i = 0; i < slots.length; i++) {
      ...
      if (displayed(whenOf(s)) && displayed(s.el)) {
        resultSeen = true;
        askConsent();
        if (consentKnown()) { render(s); continue; }
      }
      waiting++;
    }
  ...
  function mount() {
    ...
    /* The CMP finishing (or failing) can be what lets a waiting slot render. */
    afterConsent(function () { try { check(); } catch (e) { /* ignore */ } });
    check();
  }
