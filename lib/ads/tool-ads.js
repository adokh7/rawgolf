...
(function (root) {
  ...
  /* The ad-unit form of the loader, without ?client=. The client id rides on
     each unit (data-ad-client). The ?client= form is also the Auto ads code:
     on a tool page it let Auto ads place an in-page unit even with the page
     on the exclusions list, so the tool pages never load that form. */
  var ADS_SRC = 'https://pagead2.googlesyndication.com/pagead/js/adsbygoogle.js';
  ...
  /* True because every slug in PAGES is on AdSense's Auto ads page exclusions
     list (Ads > By site > Page exclusions), set by the owner on 22 Sep 2026.
     Add a page to PAGES only after it has been excluded there too. */
  var AUTO_ADS_EXCLUDED = true;
  ...
  /* A browser that sets localStorage gr_adtest = '1' asks AdSense for test ads
     (data-adtest="on"): impressions and clicks from QA are not counted. It
  ...
  function render(slot) {
    ...
    el.innerHTML = '<p class="gr-ad__label" id="' + id + '">' + LABEL + '</p>' +
      ...
      UNITS[slot.placement] + '" data-ad-format="auto" data-full-width-responsive="true"' +
