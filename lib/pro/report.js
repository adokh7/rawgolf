...
(function (root, factory) {
  ...
  function chartSvg(rows) {
    ...
    for (var t = Math.ceil(lo / 25) * 25; t <= hi; t += 25) {
      ...
      p.push('<text x="' + sx(t).toFixed(1) + '" y="' + (H - padB + 15) + '" font-size="9" fill="#55655B" text-anchor="middle" font-family="IBM Plex Mono, monospace">' + t + '</text>');
    }
    for (var r = 0; r < rows.length; r++) {
      ...
      p.push('<text x="' + (W - 4) + '" y="' + (y + 3.5) + '" font-size="10" fill="#0B1F15" text-anchor="end" font-family="IBM Plex Mono, monospace" font-weight="600">' + row.median + '</text></g>');
    }
    p.push('</svg>');
    return p.join('');
  }
