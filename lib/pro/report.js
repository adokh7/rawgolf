...
(function (root, factory) {
  ...
  /* The session's numbers are in the unit the Locker profile records them in,
     so the words follow that unit — and the two thresholds, which are yards
     like the Standing Order's, are converted rather than relabelled: 25 yards
     is 22.9 metres, never 25 metres. */
  function uWord(units, n) {
    if (units === 'meters') return n === 1 ? 'metre' : 'metres';
    return n === 1 ? 'yard' : 'yards';
  }
  function thr(units, yd) { return units === 'meters' ? Math.round(yd * 0.9144 * 10) / 10 : yd; }
  function thrWords(units, yd) {
    var v = thr(units, yd);
    return units === 'meters' ? v + ' ' + uWord(units, v) + ' (' + yd + ' yards)' : v + ' ' + uWord(units, v);
  }

  function gapVerdicts(rows, units) {
    units = units === 'meters' ? 'meters' : 'yards';
    var out = [], hole = thr(units, GAP_HOLE), dup = thr(units, GAP_DUP);
    var w = function (v) { return v + ' ' + uWord(units, v); };
    for (var i = 0; i < rows.length - 1; i++) {
      var a = rows[i], b = rows[i + 1], gap = a.median - b.median;
      if (!a.reliable || !b.reliable) continue;
      if (gap > hole) out.push({ kind: 'gap', title: 'Hole in the bag', text: w(r0(gap)) + ' between ' + a.name + ' (' + r0(a.median) + ') and ' + b.name + ' (' + r0(b.median) + '). Nothing covers the distance in between.' });
      else if (gap < dup) out.push({ kind: 'dup', title: 'Two clubs, one job', text: a.name + ' and ' + b.name + ' are ' + w(r0(gap)) + ' apart on the median; the shorter one is the usual cut.' });
      if (gap >= dup && gap <= hole && a.solid && b.solid && a.p10 < b.p90) out.push({ kind: 'dup', title: 'Overlapping dispersion', text: a.name + ' and ' + b.name + ' sit ' + w(r0(gap)) + ' apart, but their 80% bands overlap: on a given swing you cannot tell which one you hit.' });
    }
    if (!out.length && rows.length > 1) out.push({ kind: 'ok', title: 'Ladder is clean', text: 'No gap over ' + thrWords(units, GAP_HOLE) + ' and no two clubs inside ' + thrWords(units, GAP_DUP) + ' of each other.' });
    return out;
  }
  ...
  function buildModel(input) {
    input = input || {};
    var units = input.units === 'meters' ? 'meters' : 'yards';
    ...
    return {
      ...
      verdicts: gapVerdicts(rows, units),
