...
(function (root, factory) {
  ...
  function badge(el) {
    ...
    el.textContent = s === 'pro' ? 'GolfRaw Pro · active' : (s === 'preview' ? 'GolfRaw Pro · free in preview' : 'GolfRaw Pro');
  }
  ...
  function gate(container, featureKey, badgeEl) {
    ...
    return ready().then(function (status) {
      ...
      if (!existing) {
        var f = FEATURES[featureKey] || { title: 'A GolfRaw Pro feature', line: '' };
        var cover = W.document.createElement('div'); cover.className = 'gr-pro-cover';
        cover.innerHTML = '<span class="gr-pro-badge">GolfRaw Pro</span><h3>' + esc(f.title) + '</h3><p>' + esc(f.line) + '</p>' +
  ...
  function openModal(featureKey, view) {
    ...
    box.innerHTML = '<div class="gr-pro-mh"><b id="gr-pro-title">GolfRaw Pro</b><button type="button" class="gr-pro-x" aria-label="Close">&times;</button></div>' +
      '<div class="gr-pro-mb">' +
      (view === 'restore' ? '' :
