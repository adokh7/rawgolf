...
(function () {
  ...
  // (TOOLS plus LEGACY_TOOLS, pages still live but off the hub);
  ...
  var TOOLS = {
    ...
    'tools-club-distance-calculator': ['club_distance_check', 'The Distance Check', 'free'],
    'tools-scorecard-analyzer': ['round_card', 'The Round Card', 'free']
  };
  ...
  var EVENTS = {
    ...
    tool_completed: ['tool_id', 'tool_name', 'input_mode', 'anchor_type', 'game_type', 'scoring_mode',
      'round_length', 'detail_mode'],
  ...
  var ENUMS = {
    ...
    // Which kind of number a distance estimate was built from; never the number.
    anchor_type: ['driver_carry', 'iron_carry', 'swing_speed', 'handicap_band'],
    // Which Settle Up games were settled and how they were scored; never a
    // name, score, handicap, stake or balance.
    game_type: ['skins', 'nassau', 'skins_nassau'],
    scoring_mode: ['gross', 'net'],
    // Which kind of Round Card was analysed: nine or eighteen holes, and score
    // only or with putts and penalties. Never a score, course, date or hole.
    round_length: ['nine', 'eighteen'],
    detail_mode: ['quick', 'detailed']
  };
  ...
  var api = {
     ...
     *  detail.anchor_type (an enum, never a value) says what the result used;
     *  detail.game_type and detail.scoring_mode say which Settle Up games were
     *  settled and whether gross or net; detail.round_length and
     *  detail.detail_mode say what kind of Round Card it was. All are
     *  enum-checked by send(). */
    completed: function (detail) {
      if (!tool) return;
      start();
      completedAny = true;
      ...
      var p = base(); p.input_mode = mode;
      if (detail) {
        if (detail.anchor_type) p.anchor_type = detail.anchor_type;
        if (detail.game_type) p.game_type = detail.game_type;
        if (detail.scoring_mode) p.scoring_mode = detail.scoring_mode;
        if (detail.round_length) p.round_length = detail.round_length;
        if (detail.detail_mode) p.detail_mode = detail.detail_mode;
      }
