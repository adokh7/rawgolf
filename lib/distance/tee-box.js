...
(function (root) {
  ...
  function totalFromCarry(carry) {
    var f = D.USGA.carryFromTotal.driver;
    return (carry - f[0]) / f[1];
  }

  // Tee It Forward row read at a total drive; col 1 = low end, 2 = high end.
  function chartAt(total, col) {
    var rows = D.TEE_IT_FORWARD;
    if (total <= rows[0][0]) return rows[0][col];
    for (var i = 1; i < rows.length; i++) {
      if (total <= rows[i][0]) {
        return lerp(rows[i - 1][col], rows[i][col], (total - rows[i - 1][0]) / (rows[i][0] - rows[i - 1][0]));
      }
    }
  ...
  function courseRange(total) {
    var rows = D.TEE_IT_FORWARD;
    return {
      range: [r50(chartAt(total, 1)), r50(chartAt(total, 2))],
  ...
  function analyse(o) {
    ...
    var lo = c.range[0], hi = c.range[1], mid = (lo + hi) / 2;
    ...
    return {
      ...
      range: c.range, mid: mid, beyond: c.beyond,
