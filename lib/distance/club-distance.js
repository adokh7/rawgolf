...
(function (root) {
  ...
  // Tee It Forward read at total drives tLo..tHi, unrounded. Pages round once,
  // in whichever unit they show, so yards and metres never disagree.
  function teeRange(tLo, tHi) {
    ...
    return {
      course: [at(tLo, 1), at(tHi, 2)],
      beyondChart: tHi > rows[rows.length - 1][0] || tLo < rows[0][0]
    };
  }

  function teeCheck(dLo, dHi) {
    var tLo = totalFromCarry('driver', dLo), tHi = totalFromCarry('driver', dHi);
    var raw = teeRange(tLo, tHi);
    var r50 = function (x) { return Math.round(x / 50) * 50; };
    return {
      total: [tLo, tHi],
      course: [r50(raw.course[0]), r50(raw.course[1])],
      courseRaw: raw.course,
      beyondChart: raw.beyondChart
    };
  }
  ...
  var api = {
    version: 2, CLUBS: CLUBS, BANDS: BANDS, DEFAULT_BAND: DEFAULT_BAND, LIMITS: LIMITS,
    TOUR: TOUR, USGA: USGA, TEE_IT_FORWARD: TEE_IT_FORWARD,
    teeRange: teeRange, totalFromCarry: function (carry) { return totalFromCarry('driver', carry); },
