...
html = re.sub(r'<h1 class="headline">.*?</h1>', '<h1 class="headline">Michael Block Leads the Ally Challenge Into Sunday by Two</h1>', html, flags=re.DOTALL)
html = re.sub(r'<h2 class="subhead">.*?</h2>', '<h2 class="subhead">He\'s in on an invitation, his son is on the bag, and he opened Saturday with an eagle. What Block actually needs from Sunday, and it isn\'t the trophy.</h2>', html, flags=re.DOTALL)
